...
describe('interleave', function() {
  ...
  it.skip('next', function() {
    ...
    seq.next().should.eql([ 2 ]);
    seq.next().should.eql([ 'b' ]);
  })

  it.skip('complete sequence', function() {
    var seq = new LazySeq({ 'x': data1, 'y': data2 }).interleave(); 
  ...
  it.skip('reset sequence', function() {
    var seq = new LazySeq({ y: data2, x: data1 }).interleave();
  ...
  it.skip('priority ordering', function() {
    var seq = new LazySeq({ 'a': data1, 'z': data2 }).interleave();
    seq.next().should.eql({ 'a': 1, 'z': 'a' });
    seq.next().should.eql({ 'a': 2, 'z': 'a' });

    var seq = new LazySeq({ 'z': data1, 'a': data2 }).interleave();
